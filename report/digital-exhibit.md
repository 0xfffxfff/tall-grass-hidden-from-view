...
4. After a move, the oracle reports the terrain at your new cell (tall grass or clear ground) and any encounter. There is no map — the landscape only emerges through the monolith's progressive reveal.
...
The landscape is a toroidal grid whose terrain — tall grass and clear ground, in roughly equal measure — is committed on-chain at deployment as a Merkle root, then never revealed publicly. Participants connect a wallet, receive a starting position from a sealed oracle, and move one cell at a time. Every move is a zero-knowledge proof generated in the participant's browser and verified on-chain: the contract sees that a move was valid without ever learning where it happened. Positions live as Poseidon commitments. Direction is private.
