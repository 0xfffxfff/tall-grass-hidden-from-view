...
1. **Commitments to a hidden landscape.** A Poseidon hash of the landscape seed (`seedCommitment`) and a per-cell Merkle root over `Poseidon(x, y, terrain_type)` leaves (`terrainMerkleRoot`) are deployment-time constants. Together they bind the artist to a specific terrain map without revealing it. The map can never be retroactively edited.
...
2. **ZK-verified participant movement.** Every move is a Noir-generated zero-knowledge proof, generated in the participant's browser, verified on-chain by a Solidity verifier (Honk verifier emitted by `bb`). The proof attests:
   - the prior on-chain `positionCommitment` opens to some `(x, y, salt)`,
   - the new commitment opens to `(x', y', salt')`,
   - `(x', y')` is exactly one cardinal step from `(x, y)` with toroidal wrapping on the public `gridWidth`/`gridHeight`,
...
5. **On-chain encounter verification.** Minting requires a Noir proof (the `encounter` circuit) that the participant's current `positionCommitment` co-locates with an entity position derived from the committed seed. Public inputs include `seedCommitment`, `entityId`, `positionCommitments[msg.sender]`, `gridWidth`, `gridHeight`, the entity's `initialPositionCommitment`, and `blindingSeedCommitment`. The contract verifies the proof, the Merkle inclusion of the trait CID, and the mint payment in a single atomic transaction. Encounters cannot be faked: a participant has to actually be at the entity's cell to mint.
...
- A movement verifier contract for participant moves.
- A movement verifier for entity moves (post-mint, blinded direction).
- An encounter verifier that proves seed-derived co-location.
- A Poseidon commitment over the FHE secret key, paired with a decryption-correctness verifier consumed client-side.
...
**A patent-free, browser-runnable TFHE comparison stack** (`fhe-wasm/`). The dominant open-source FHE library — Zama's TFHE-rs — ships under BSD 3-Clause Clear, a license with an explicit patent carve-out: "no express or implied licenses to any party's patent rights are granted by this license." Zama holds six US patents and a European patent covering optimization techniques used in their library, and has actively asserted them against competitors (notably Sunscreen). For an artwork on Ethereum mainnet that charges a mint fee, the line between Zama's permitted "experimentation" and prohibited "commercial use" is not clear. *Tall Grass* therefore vendors and builds the **original TFHE C library v1.1 (Apache 2.0, patent-free)** to WebAssembly via emscripten 5.0.0, replacing the x86-specific FFTW3 with the pure-C Nayuki portable FFT. The result is a 198 KB WASM binary that performs 8-bit homomorphic comparisons (GT, EQ) entirely in the browser, with cross-environment verification (native keygen → WASM compare → native decrypt). I am not aware of an earlier patent-free, browser-runnable TFHE stack. Patent context documented at `context/research/zama-patents.md`.

**Verifiable FHE comparison via ZK proof of correct decryption** (`circuits/decryption/`). A homomorphic comparison result returned by an oracle is normally trusted. *Tall Grass* turns each result into a verifiable claim: when the oracle decrypts the boolean ciphertext, it produces a Noir zero-knowledge proof attesting that the decryption was performed under the secret key whose Poseidon commitment was fixed on-chain at deployment. The browser verifies the proof locally against the on-chain commitment. The trust model shifts from "the oracle is honest" to "the oracle is provably honest, per comparison." The construction is reusable wherever FHE is used to make claims about hidden data that need to be auditable.
...
| FHE | TFHE C library | v1.1, Apache 2.0 | Original TFHE C, *not* TFHE-rs (avoids the Zama patent concern, see `context/research/zama-patents.md`). Compiled to WASM via emscripten 5.0.0 with the Nayuki portable FFT (no x86 dependencies). Lambda=80 security parameter. |
