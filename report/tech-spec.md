...
A programmable cryptography artwork by 0xfff. Funded by The Pixel Prize, JUST Open Source Stiftung.
...
1. **Commitments to a hidden landscape.** A Poseidon hash of the landscape seed ([`seedCommitment`](https://github.com/0xfffxfff/tall-grass-hidden-from-view/blob/main/contracts/src/TallGrass.sol#L63)) and a per-cell Merkle root over `Poseidon(x, y, terrain_type)` leaves ([`terrainMerkleRoot`](https://github.com/0xfffxfff/tall-grass-hidden-from-view/blob/main/contracts/src/TallGrass.sol#L64)) are deployment-time constants. Together they bind the artist to a specific terrain map without revealing it. The map can never be retroactively edited.
...
4. **FHE-encrypted entity traits with merkle commitments.** Entity ciphertexts (TFHE, ~110 KB per entity for 7 traits) are committed on-chain at deployment as an [`entityTraitMerkleRoot`](https://github.com/0xfffxfff/tall-grass-hidden-from-view/blob/main/contracts/src/TallGrass.sol#L65) with leaves ordered by entity index. Pre-mint, ciphertexts are served from IPFS for encounter and comparison flows; every served ciphertext is checked against the on-chain root via a Merkle proof. On mint, the relevant ciphertext is written to the token's metadata contract via chunked `SSTORE2` so the encrypted traits live permanently on-chain alongside the NFT. The traits themselves are *permanently encrypted* — no decryption flow exists. Only homomorphic comparison is possible.

5. **On-chain encounter verification.** Minting requires a Noir proof (the [`encounter`](https://github.com/0xfffxfff/tall-grass-hidden-from-view/tree/main/circuits/encounter) circuit) that the participant's current `positionCommitment` co-locates with an entity position derived from the committed seed. Public inputs include `seedCommitment`, `entityId`, `positionCommitments[msg.sender]`, `gridWidth`, `gridHeight`, the entity's `initialPositionCommitment`, and [`blindingSeedCommitment`](https://github.com/0xfffxfff/tall-grass-hidden-from-view/blob/main/contracts/src/TallGrass.sol#L95). The contract verifies the proof, the Merkle inclusion of the trait hash, and the mint payment in a single atomic transaction. Encounters cannot be faked: a participant has to actually be at the entity's cell to mint.

6. **ZK proofs of correct FHE decryption.** When the oracle decrypts the boolean output of a homomorphic comparison, it emits a Noir proof attesting that the decryption was performed under the secret key whose Poseidon commitment ([`decryptionKeyCommitment`](https://github.com/0xfffxfff/tall-grass-hidden-from-view/blob/main/contracts/src/TallGrass.sol#L74)) was fixed at deployment. The browser verifies this proof locally against the on-chain commitment. The oracle's honesty is enforced per comparison.
...
These verifiers turn the chain into a sealed witness for cryptographic facts about a world that is not on the chain. The NFT is the receipt; the verification is the work.
...
The piece treats the chain the way a sculptor treats a load-bearing armature: not the surface of the work, but the geometry that makes its form possible.

### 2.4 Positioning relative to verifiable computation

Most production zero-knowledge cryptography today is used for verifiable computation in service of blockchain throughput — rollup state transitions, sequencer validity proofs, off-chain compute attestations. The label "zero-knowledge" has drifted from its original sense (proving a statement without revealing the witness) toward "proof of correct execution," with the privacy-preserving aspect largely incidental. *Tall Grass* uses zero-knowledge for the property the math actually promised: hiding the witness. The chain learns only that valid moves occurred; positions, directions, and the landscape itself remain hidden.
...
**A patent-free, browser-runnable TFHE comparison stack** ([`fhe-wasm/`](https://github.com/0xfffxfff/tall-grass-hidden-from-view/tree/main/fhe-wasm)). The dominant open-source FHE library — Zama's TFHE-rs — ships under BSD 3-Clause Clear, a license with an explicit patent carve-out: "no express or implied licenses to any party's patent rights are granted by this license." Zama holds six US patents (12,517,975; 12,418,397; 12,375,260; 12,177,330; 12,143,467; 11,991,266 — all covering specific optimizations such as NTT-based blind rotation, programmable bootstrapping, radix decomposition, and parameter selection) and a European patent (EP4150852B1, granted June 2024) over the lookup-table technique known as "programmable bootstrapping." Notably, the equivalent US application for EP4150852B1 was rejected by the USPTO as an abstract mathematical method (post–*Alice v. CLS Bank*); the EPO granted it under European doctrine treating cryptography as having "technical character" even when claims are fundamentally mathematical. The 9-month EPO opposition window closed in March 2025; remaining challenge routes (national invalidity actions, UPC revocation) require organizational backing. Zama has actively asserted these rights against competitors — sending legal threats to Sunscreen in December 2023 over programmable bootstrapping and "disparaging technical comparisons." For an artwork on Ethereum mainnet that charges a mint fee, the line between Zama's permitted "experimentation" and prohibited "commercial use" is not clear. *Tall Grass* therefore vendors and builds the **original TFHE C library v1.1 (Apache 2.0, patent-free; Chillotti et al., the academic implementation that predates Zama's patents)** to WebAssembly via emscripten 5.0.0, replacing the x86-specific FFTW3 with the pure-C Nayuki portable FFT. The result is a 198 KB WASM binary that performs 8-bit homomorphic comparisons (GT, EQ) entirely in the browser, with cross-environment verification (native keygen → WASM compare → native decrypt). Alternatives evaluated and rejected: OpenFHE (BSD-2-Clause, no patent carve-out, but the WASM port omits the TFHE/CGGI scheme and is Node-only); Order-Revealing Encryption (leaks full ordering passively, eliminates the "computational weight" of comparison that is part of the work); ZK-only comparison via the existing oracle (no FHE infrastructure, but adds no capability the oracle does not already have). I am not aware of an earlier patent-free, browser-runnable TFHE stack.
...
| Wallet | wagmi + viem | Reown (WalletConnect) as backup connector |
...
- Mac Mini (M-series) hosts the oracle, the chain monitor, and the WebGL/Three.js fullscreen visual renderer; HDMI to both displays.
...
- **Proving runs in the participant's browser, with no prover service in the loop.** Production zero-knowledge systems increasingly rely on centralized GPU prover farms — proving-as-a-service infrastructure that can refuse, delay, or filter specific users, and that becomes the de facto trust root for systems claiming trustlessness. *Tall Grass* generates every participant move proof client-side in WASM (`bb.js` UltraHonk). Where the math promises trustlessness at the prover layer, the implementation honors it.
